...
double min_max(double v, double minA, double maxA, double new_minA, double new_maxA)
{
    double vd = (((v - minA) / (maxA - minA)) * (new_maxA - new_minA)) + new_minA;
    return vd;
}
...
double Decimal_Scaling(double v, int j)
{
    double vd = v / pow(10, j);
    return vd;
}

void call_min_max(vector<double> v1, vector<double> &v2)
{
    double minA = *min_element(v1.begin(), v1.end());
    double maxA = *max_element(v1.begin(), v1.end());

    double new_minA, new_maxA;

    cout<<"Enter new min and max: ";
    cin>>new_minA>>new_maxA;

    for(auto elm: v1)
        v2.push_back(min_max(elm, minA, maxA, new_minA, new_maxA));
...
void call_z_score(vector<double> v1, vector<double> &v2)
{
    ...
    // cout<<"Mean: "<<m<<endl<<"SD: "<<sd<<endl;
    for(auto elm: v1)
        v2.push_back(z_score(elm, m, sd));
    cout<<"Z-score normalization: ";
}

void call_decimal_scaling(vector<double> v1, vector<double> &v2)
{
    cout<<"Decimal point: ";
    int j=0;
    cin>>j;
    for(auto elm: v1)
        v2.push_back(Decimal_Scaling(elm, j));
...
int main()
{
    ...
    cout<<"Choose methode for normalization : "<<endl;
    cout<<"1. Min-Max normalization \n2. Z-score normalization \n3. Decimal scaling normalization"<<endl;
